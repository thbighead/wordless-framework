...
class FailedAggregatingObject extends WpErrorException
{
    ...
    )
    {
        if ($requestError === false) {
            $requestError = new WP_Error(
                'unknown_error',
                "Unknown error when aggregating {$this->term->name} ({$this->term->taxonomy} of ID {$this->term->id()}) to object of ID {$this->object->id()}."
            );
        }
