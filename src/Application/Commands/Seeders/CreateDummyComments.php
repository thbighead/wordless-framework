...
class CreateDummyComments extends BaseCreateDummyCommand
{
    ...
    protected function runIt(): int
    {
        $this->wrapScriptWithMessages('Creating Comments...', function () {
            ...
            foreach (Post::getAll() as $post) {
                for ($i = 0; $i < $this->getTotalCommentsToCreate(); $i++) {
                    $post_id = $post->ID;
    ...
    private function getTotalCommentsToCreate(): int
    {
        return (int)(($this->input->getOption(self::OPTION_TOTAL) ?: null) ?? self::HOW_MANY_COMMENTS_PER_POST);
    }
}
